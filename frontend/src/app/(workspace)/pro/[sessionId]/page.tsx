...
import { getFileRawUrl, getFileDownloadUrl, IMAGE_EXTS, type WorkspaceFile as WsFile } from '@/shared/lib/workspace-files-api'
...
function FilePreviewPanel({ file, onClose }: { file: WsFile & { workspaceId: string }; onClose: () => void }) {
  ...
  const rawUrl = getFileRawUrl(file.workspaceId, file.path)
  const dlUrl  = getFileDownloadUrl(file.workspaceId, file.path)

  useEffect(() => {
    if (!isText || isHtml) return
    setLoading(true)
    fetch(rawUrl).then(r => r.text()).then(t => setTextContent(t)).catch(() => setTextContent('无法加载文件内容')).finally(() => setLoading(false))
  }, [rawUrl, isText, isHtml])

  return (
    <div className="flex flex-col h-full">
      {/* 标题栏 */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 shrink-0 bg-gray-50/50">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-sm">{isHtml ? '🌐' : isImg ? '🖼️' : isText ? '📄' : '📎'}</span>
          <span className="text-xs font-medium text-gray-700 truncate">{file.name}</span>
          <span className="text-[10px] text-gray-400 font-mono shrink-0">{file.path}</span>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <a href={dlUrl} download={file.name}
            className="text-[10px] px-2 py-1 rounded-md text-gray-400 hover:text-orange-500 hover:bg-orange-50 transition-colors">
            下载
          </a>
          <button onClick={onClose}
            className="w-6 h-6 flex items-center justify-center rounded-md text-gray-300 hover:text-gray-600 hover:bg-gray-100 transition-colors text-xs">
            ✕
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        {isHtml ? (
          <iframe src={rawUrl} className="w-full h-full border-0" title={file.name} sandbox="allow-scripts allow-same-origin" />
        ) : isImg ? (
          <div className="flex items-center justify-center h-full p-6 bg-gray-50">
            <img src={rawUrl} alt={file.name} className="max-w-full max-h-full object-contain rounded-xl shadow-sm" />
          </div>
        ) : isText ? (
          loading ? (
            <div className="flex items-center justify-center h-full"><span className="text-xs text-gray-400">加载中…</span></div>
          ) : (
            <pre className="p-4 text-[11px] font-mono text-gray-700 leading-relaxed whitespace-pre-wrap break-all">{textContent}</pre>
          )
        ) : (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center p-8">
            ...
            <a href={dlUrl} download={file.name}
              className="text-xs px-3 py-1.5 rounded-lg bg-orange-50 text-orange-500 hover:bg-orange-100 transition-colors border border-orange-100">
              下载文件
            </a>
          </div>
        )}
...
export default function ProSessionPage() {
  ...
  const [previewFile, setPreviewFile]   = useState<(WsFile & { workspaceId: string }) | null>(null)
  // 历史恢复状态（防止重复加载）
  ...
  // ABC 更新时自动切换到预览 Tab
  useEffect(() => {
    if (abcNotation) { setCenterTab('preview'); setPreviewFile(null) }
  }, [abcNotation])
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-white border-r border-gray-100 min-w-0">
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              abcNotation ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center p-10 space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-gray-400 max-w-xs leading-relaxed">直接告诉 AI 你想做什么</p>
                  </div>
            ...
            {!previewFile && centerTab === 'logs' && <div className="p-4"><PipelineStatus /></div>}
            {!previewFile && centerTab === 'export' && <ExportPanel />}
          </div>
        </main>
