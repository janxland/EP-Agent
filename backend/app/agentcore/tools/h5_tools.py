...
# 旧硬编码模板名集合（仅用于向后兼容判断，不再扩展）
# 新增模板只需在 h5-templates/ 下建文件夹 + index.html + meta.json，无需修改此处
_LEGACY_TEMPLATES = {"apple_dark", "apple_light", "minimal"}


def _is_file_template(name: str) -> bool:
    """
    判断是否为文件级模板（走 render_score_to_h5 路径）。
    旧硬编码模板名（apple_dark / apple_light / minimal）强制走旧路径，
    其余名称只要注册表中存在对应文件夹即为文件级模板。
    """
    if name in _LEGACY_TEMPLATES:
        return False  # 旧硬编码模板，强制走 build_h5_html
    from app.agentcore.tools.h5_renderer import _REGISTRY_BY_NAME, _LEGACY_ALIASES
    resolved = _LEGACY_ALIASES.get(name, name)
    return resolved in _REGISTRY_BY_NAME
...
def generate_h5_poster(
    ...
    # 动态判断：注册表中存在则走文件级模板，否则降级旧硬编码
    if _is_file_template(template):
        try:
            ...
            html = build_h5_html(
                title=title, notes=notes_list, template="apple_dark",
                source_format=source_format, abc_content=abc_content,
                bpm=bpm, key=key, composer=composer, extra_info=extra_info,
            )
    else:
        html = build_h5_html(
            title=title, notes=notes_list, template=template,
            source_format=source_format, abc_content=abc_content,
            bpm=bpm, key=key, composer=composer, extra_info=extra_info,
        )
...
def generate_h5_from_abc(
    ...
    if _is_file_template(template):
        try:
            ...
            html = build_h5_html(
                title=title, notes=notes, template="apple_dark",
                source_format="abc", abc_content=abc,
                bpm=bpm, key=key, composer=composer, extra_info=extra_info,
            )
    else:
        html = build_h5_html(
            title=title, notes=notes, template=template,
            source_format="abc", abc_content=abc,
            bpm=bpm, key=key, composer=composer, extra_info=extra_info,
        )
...
def generate_h5_from_midi(
    ...
    try:
        ...
        html = build_h5_html(
            title=fallback_title, notes=[],
            template="apple_dark", source_format="midi",
            composer=composer, extra_info=extra_info,
        )
