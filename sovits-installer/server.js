...
const SKIP_BROWSER_OPEN = process.env.SOVITS_SKIP_BROWSER_OPEN === '1';
...
function runCommand(cmd, args, cwd, onLine) {
  return new Promise((resolve, reject) => {
    const opts = { cwd: cwd || process.cwd(), shell: true, env: { ...process.env, TERM: process.env.TERM || 'xterm' } };
...
async function runInstall(config) {
  ...
  const steps = [
    ...
    },
    {
      ...
      run: async () => {
        if (IS_MAC) {
          ...
          const aptCmd = typeof process.getuid === 'function' && process.getuid() === 0 ? 'apt-get' : 'sudo';
          const aptArgs = aptCmd === 'apt-get'
            ? ['update', '&&', 'apt-get', 'install', '-y', 'ffmpeg', 'libsox-dev']
            : ['apt-get', 'install', '-y', 'ffmpeg', 'libsox-dev'];
          broadcast('log', { level: 'cmd', msg: `$ ${aptCmd} ${aptArgs.join(' ')}` });
          await runCommand(aptCmd, aptArgs, null).catch(() =>
    ...
    },
    {
      ...
      run: async () => {
        const envExists = await new Promise(resolve => {
          exec('conda env list', (err, stdout) => {
            resolve(!err && new RegExp(`^GPTSoVits\\s`, 'm').test(stdout));
          });
        });
        if (envExists) {
          broadcast('log', { level: 'success', msg: 'Conda 环境 GPTSoVits 已存在，跳过创建' });
          return;
        }
    ...
    },
    {
      ...
      run: async () => {
        if (IS_WIN) {
          ...
          const dev = (device || 'CPU').toUpperCase();
          const args = ['run', '-n', 'GPTSoVits', 'env', 'TERM=xterm', 'bash', 'install.sh',
            '--device', dev, '--source', source];
    ...
    },
    {
      ...
      run: async () => {
        const verifyCmd = 'conda run --no-capture-output -n GPTSoVits python -c "import torch; print(torch.__version__)"';
        broadcast('log', { level: 'cmd', msg: '$ ' + verifyCmd });
        await runCommand(verifyCmd, [], absDir);
        // 检查模型目录
...
async function checkWebuiOnline() {
  return (await checkPort(9872)) || (await checkPort(9874));
}
...
async function runUpdate(config) {
  ...
  const steps = [
    ...
    },
    {
      ...
      run: async () => {
        ...
        if (IS_WIN) {
          ...
          const args = ['run', '-n', 'GPTSoVits', 'env', 'TERM=xterm', 'bash', 'install.sh',
            '--device', dev.toUpperCase(), '--source', src];
          if (uvr5Flag) args.push(uvr5Flag);
          broadcast('log', { level: 'cmd', msg: '$ conda ' + args.join(' ') });
          await runCommand('conda', args, absDir);
        }
    ...
    },
    {
      ...
      run: async () => {
        await runCommand('conda run --no-capture-output -n GPTSoVits python -c "import torch; print(torch.__version__)"', [], absDir);
        // 获取最新 commit
  ...
  if (req.method === 'POST' && url === '/launch') {
    ...
    req.on('end', async () => {
      ...
      if (appState.webuiRunning) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: false, msg: 'WebUI 已在运行中', url: appState.webuiUrl || 'http://localhost:9874' }));
        return;
      }

      // 启动前先 kill 掉常用 WebUI 端口，确保端口固定
      broadcast('log', { level: 'info', msg: '  检查端口 9872/9874 占用情况...' });
      await killPort(9872);
      await killPort(9874);
      // 等待端口释放
      ...
      appState.webuiUrl = 'http://localhost:9874';
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, url: appState.webuiUrl, pid: child.pid }));
    });
    return;
  }
